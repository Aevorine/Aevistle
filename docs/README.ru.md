...
| 📆 **Рабочий календарь, который показывает свой эффект** | Праздники, выходные, которые вы задаёте сами, и рабочие субботы — с сеткой месяца, где день переключается щелчком, готовыми наборами для шести стран и числом напоминаний на каждой дате; закрашенным, если именно календарь их туда перенёс. |
...
|---|---|---|
| Windows 10/11 (x64) | `Aevistle-0.1.4-win-x64-setup.exe` | Установщик, создаёт ярлыки в меню «Пуск» и на рабочем столе |
| Windows 10/11 (x64) | `Aevistle-0.1.4-win-x64-portable.exe` | Один файл, без установки, работает с флешки |
| Android 7.0+ | `Aevistle-0.1.4.apk` | Телефоны и планшеты. Сначала разрешите «установку неизвестных приложений» для браузера или файлового менеджера. |
